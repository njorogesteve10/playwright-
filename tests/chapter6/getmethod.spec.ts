...
test('Create TypeSafety POST API Request using dynamic api request body in playwright & typescript', async ({ request }) => {
    ...
    expect(jsonPOSTAPIResponse.booking.bookingdates.checkout).toBe('2025-01-27');
     ...
     const bookingId = await jsonPOSTAPIResponse.bookingid;
     const answer = await request.get(`/booking/`{
        params :{
            firstname: firstName,
            lastname: lastName
        }
     ...
     expect(answer.statusText()).toBe('OK');
     
     
     const apiresponseget = await answer.json();
     console.log("result from get is" + JSON.stringify(apiresponseget));
});